...
# from model import Generator_3 as Generator
# RIFB MODIFCATION
from model import Generator_3_RFIB as Generator
from cost_functions import get_KLdivergence_loss, get_RFIB_loss
from model import InterpLnr
import matplotlib.pyplot as plt
import torch
import torch.nn.functional as F
...
class Solver(object):
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            # RFIB MODIFICATION
            try:
                # CHANGED: Now unpack the sensitive attribute s from the loader as well.
                x_real_org, emb_org, f0_org, s, len_org = next(data_iter)
            ...
            # RIFB MODIFICATION
            s = s.to(self.device)  # CHANGED: Move sensitive attribute to device
            ...
            # IB
            # g_loss_id = get_KLdivergence_loss(x_real_org, x_identic, mu, var, beta=100, r_method='mean')

            # RFIB
            if isinstance(s, list) or (hasattr(s, '__iter__') and isinstance(s[0], str)):
                mapping = {"F": 0, "M": 1}
                s = torch.tensor([mapping[val] for val in s], dtype=torch.long)
            else:
                s = s.long()
            x_identic, x_identic_fair, var, mu = self.G(x_f0_intrp_org, x_real_org, emb_org, s)
            g_loss_id = get_RFIB_loss(x_real_org, x_identic_fair, x_identic, mu, var, alpha=1, beta1=50, beta2=0.01)
            ...
            if (i+1) % self.sample_step == 0:
                # Load the spk2gen mapping (assumes a pickle file mapping speaker IDs to gender strings)
                ...
                with torch.no_grad():
                    ...
                    for val_sub in validation_pt:
                        ...
                        # Derive the sensitive attribute s using speaker_id.
                        # Here, we map gender: 'F' -> 0, 'M' -> 1.
                        gender = spk2gen[speaker_id]
                        s_val = torch.tensor([0 if gender == 'F' else 1]).to(self.device)

                        # Forward pass through the generator.
                        # Note that the generator now takes s_val as its fourth argument.
                        x_identic_val, x_identic_fair_val, _, _ = self.G(x_f0, x_real_pad, emb_org_val, s_val)

                        # Compute reconstruction loss (MSE) between the original mel and the generated fair output.
                        # IB
                        # g_loss_val = F.mse_loss(x_real_pad, x_identic_val, reduction='sum')
                        
                        # RFIB
                        g_loss_val = F.mse_loss(x_real_pad, x_identic_fair_val, reduction='sum')
